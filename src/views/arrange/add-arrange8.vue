    ...
    <div>
        ...
        <div class="right_wrap h100" v-if="show_page">
            ...
            <div>
                ...
                <div class="right_box">
                    <div class="div1">
                        <el-select v-model="ruleForm.unine.act_type" placeholder="请选择活动类型">
                            <el-option :label="item.desc" :value="item.act_type" v-for="(item,index) in act_list" :key="index"></el-option>
                        </el-select>
                        <el-input v-model="ruleForm.unine.act_id" placeholder="填写活动id"></el-input>
                    </div>
                </div>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="right_box">
                    <div v-for="(sitem,sindex) in ruleForm.roller_list1" :key="sindex" class="div1">
                        <p>
                            <span class="text">秒杀场次{{sindex + 1}}</span>
            ...
            <div>
                ...
                <div class="right_box">
                    <div v-for="(sitem,sindex) in ruleForm.wan_data" :key="sindex" class="div1">
                        <p>
...
export default {
    ...
    created() {
        cmsGetData({ id: 164, tag: 'YEARACT8' }).then(res => {
            ...
            for (let i = 0; i < this.ruleForm.roller_list1.length; i++) {
                this.ruleForm.roller_list1[i].end_time = this.ruleForm.roller_list1[i].end_time * 1000
                this.ruleForm.roller_list1[i].start_time = this.ruleForm.roller_list1[i].start_time * 1000
            }
        })
        actType().then(res => {
    ...
    methods: {
        submitForm(formName) {
            ...
            timeout = setInterval(function () {
                console.log(that.ruleForm.roller_list[that.ruleForm.roller_list.length - 1].start_time)
                console.log(that.ruleForm.roller_list1[that.ruleForm.roller_list1.length - 1].start_time)
                if (that.ruleForm.roller_list[that.ruleForm.roller_list.length - 1].start_time.toString().length < 13 && that.ruleForm.roller_list1[that.ruleForm.roller_list1.length - 1].start_time.toString().length < 13) {
        ...
        addRollerList1() {
            this.$confirm('确定新增场次吗', '提示', {
                ...
                this.ruleForm.roller_list1.push({
                    act_type: '6',
                    act_id: '',
                    start_time: '',
                    end_time: '',
        ...
        deleteRollerList1(index) {
            this.$confirm('确定删除场次吗', '提示', {
                ...
                this.ruleForm.roller_list1.splice(index, 1)
            })
        },
        addZongData() {
