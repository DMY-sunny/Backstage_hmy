<template>
    <div style="background:#fff">
        ...
        <el-table :data="tableData" border style="width: 100%">
            <el-table-column prop="name" label="入口标题" width="180"></el-table-column>
            <el-table-column prop="en_img" label="入口图" width="200">
                <template slot-scope="scope">
                    <img style="width:100%;height:auto" :src="cdn06 + scope.row.en_img" />
                </template>
            </el-table-column>
            <el-table-column prop="show_link" label="入口链接_H5" width="200"></el-table-column>
            <el-table-column prop="show_link" label="入口链接_小程序" width="200" :formatter="formatXcxUrl"></el-table-column>
            <el-table-column prop="jump" label="自动跳转">
                <template slot-scope="scope" width="80">
                    <div>{{scope.row.jump == 1 ? '是' : '否'}}</div>
                </template>
            </el-table-column>
            <el-table-column prop="bottom_flag" label="子入口展示">
                <template slot-scope="scope" width="80">
                    <div>{{scope.row.bottom_flag == 1 ? '展示' : '不展示'}}</div>
                </template>
            </el-table-column>
            <el-table-column prop="act_flag" label="商品展示框">
                <template slot-scope="scope" width="80">
