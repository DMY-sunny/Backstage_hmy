    ...
    <div class="container">
        ...
        <el-table :data="tableData"
            ...
            <!-- <el-table-column prop="role_id" label="所属角色ID" align="center">
                <template slot-scope="scope">
                    <el-tooltip class="item" :key="index"  effect="dark" content="点击可注销角色" placement="top" v-for="(item,index) in scope.row.roles">
                        <el-button style="margin: 1px;" type="danger" plain @click="logoutrole(item.role_id,scope.row.login_id)">{{item.role_id}}</el-button>
                    </el-tooltip>
                </template>
            </el-table-column>
            <el-table-column prop="name" label="所属角色标识" align="center">
                <template slot-scope="scope">
                    <el-tag style="margin: 1px;" v-for="(item,index) in scope.row.roles" :key="index" type="success" effect="dark">{{item.role_id}}~{{item.name}}</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="name" label="所属角色描述" align="center">
                <template slot-scope="scope">
                    <el-tag style="margin: 1px;" v-for="(item,index) in scope.row.roles" :key="index" type="warning" effect="dark">{{item.role_id}}~{{item.desc}}</el-tag>
                </template>
            </el-table-column> -->
