...
<div class="table">
    ...
    <el-dialog title="角色信息" :visible.sync="createUserVisible" width="80%" center @close="cancel('create_user')">
        <el-form ref="create_user" :rules="rules" :model="create_user" label-width="110px">
            ...
            <el-form-item v-if="createvsupdate == 1" label="角色类型">
...
export default {
    ...
    methods: {
        ...
        handlePermis(id){
            ...
            Cpermissions(id,this.cur_page,100).then(res=>{
                if(res.errno == 0){
                    this.PermisData = res.data.list;//当前角色下的权限
                    Plist(this.cur_page,100).then(res => {
                        if (res.errno == 0) {//所有权限
...
//        取消
        ...
        async submitPermis(){
            this.roleChange();
            console.log(this.permischeckbox)
            let sendObj = {
